...
export default function App() {
  ...
  // Strict physical updates to enforce unmute/play across React DOM updates
  useEffect(() => {
    ...
    if (heroVideoRef.current) {
      if (heroPlaying) {
        heroVideoRef.current.play().catch((err) => {
          console.log("Play failed in state watcher:", err);
        });
      } else {
  ...
  // Autoplay handler & capture-phase global first gesture unmute trigger
  useEffect(() => {
    const video = heroVideoRef.current;
    if (!video) return;

    // Start with sound on state by default
    video.muted = false;
    setHeroMuted(false);
    ...
    if (playPromise !== undefined) {
      playPromise
        ...
        .catch((error) => {
          console.log("Unmuted autoplay restricted by browser, fallback to physically muted autoplay:", error);
          // PHYSICALLY mute the video element so browser allows play, but do NOT set React state heroMuted to true!
          // This keeps the sound icon state visually pink / ON so user knows sound is supposed to be active.
          video.muted = true;
          video.play()
            .then(() => {
              setHeroPlaying(true);
    ...
    // Unmute on first gesture anywhere on the document (the ultimate seamless trigger)
    const handleFirstUserGesture = (evt: Event) => {
      // If user clicked directly on mute/play control buttons, do not override their intent
      const target = evt.target as HTMLElement;
      if (target && target.closest('.video-control-btn')) {
        return;
      }
      ...
      if (video) {
        ...
        video.play()
          ...
          .catch(e => console.log("Gesture unmute play failure:", e));
      }

      // Cleanup immediately so it only triggers on the absolute first interaction anywhere
      ...
      document.removeEventListener('mousedown', handleFirstUserGesture, true);
    ...
    document.addEventListener('mousedown', handleFirstUserGesture, true);
  ...
  const handleToggleHeroPlay = (e: React.MouseEvent) => {
    e.stopPropagation();
  ...
  return (
    <div id="landing-page-root" className="min-h-screen bg-[#050505] text-gray-100 font-sans selection:bg-pink-500 selection:text-white">
      ...
      <header className="relative pt-2 pb-16 md:pt-4 md:pb-24 overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-4 md:px-8 relative z-10">
          ...
          <div className="text-center max-w-4xl mx-auto space-y-6 pt-2">
            ...
            >
              {/* Overlay frames / lighting gradient */}
              ...
              {/* Video control badges & overlays (Play/Pause, Sound) */}
              <div className="absolute inset-0 flex flex-col justify-end p-4 md:p-6 z-20 pointer-events-none">
                ...
                <div className="flex justify-between items-end">
                  ...
                  <div className="text-left hidden sm:block">
                    <p className="text-[10px] uppercase text-pink-400 font-mono font-bold tracking-wider">Under The Hood</p>
                    <h3 className="text-sm font-bold text-white font-display">Watch listing automation script running</h3>
                  </div>
                  ...
                  <div className="flex items-center gap-2.5 ml-auto pointer-events-auto">
                    ...
                    <button
                      ...
                      className={`video-control-btn p-3 rounded-full backdrop-blur-md border transition-all cursor-pointer shadow-lg hover:scale-105 duration-200 ${
                        heroPlaying
                          ? "bg-[#050110]/80 border-purple-500/30 text-pink-400"
                          : "bg-purple-600 border-purple-500 text-white shadow-[0_0_15px_rgba(147,51,234,0.5)] animate-pulse"
                      }`}
                      title={heroPlaying ? "Pause Video" : "Play Video"}
                    >
                      {heroPlaying ? <Pause className="w-4 h-4 text-pink-400" /> : <Play className="w-4 h-4 text-white fill-white" />}
                    </button>
                  </div>
      ...
      <div className={`fixed bottom-4 left-1/2 -translate-x-1/2 z-45 w-[95%] max-w-lg md:max-w-2xl px-1 transition-all duration-300 ${!isHeroButtonVisible ? "opacity-100 translate-y-0 pointer-events-auto" : "max-md:opacity-0 max-md:translate-y-10 max-md:pointer-events-none"}`}>
      ...
      <a
        ...
        target="_blank"
        rel="noopener noreferrer"
        className={`fixed right-4 z-40 bg-emerald-500 hover:bg-emerald-400 text-white p-3 md:p-3.5 rounded-full shadow-[0_8px_24px_rgba(34,197,94,0.35)] duration-300 hover:scale-110 flex items-center justify-center cursor-pointer transition-all border border-emerald-400/25 pulse-glow ${
          !isHeroButtonVisible 
