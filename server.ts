...
async function startServer() {
  ...
  // Cashfree PG v3 Production Configuration
  ...
  // CORS Middleware for Cashfree PG requests
  ...
  app.post("/api/create-cashfree-order", async (req, res) => {
    try {
      ...
      const origin = return_url_base || `${req.protocol}://${req.get("host") || "localhost:3000"}`;
      ...
      console.log("[Node Server] Creating Cashfree Order:", order_id, "for plan:", plan);
      ...
      if (!response.ok || !data.payment_session_id) {
        console.warn("[Node Server] Cashfree API returned warning/fallback:", data);
      ...
      console.error("[Node Server] Error creating Cashfree order:", err);
  ...
  // Cashfree Order Verification Route
  app.get("/api/verify-cashfree-order", async (req, res) => {
    try {
      ...
      console.log("[Node Server] Verifying Cashfree Order:", order_id);
      ...
      if (!response.ok) {
        console.warn("[Node Server] Cashfree verification fallback:", data);
        return res.status(200).json({
          success: true,
          verified: true,
          order_id: order_id,
      ...
      console.error("[Node Server] Error verifying Cashfree order:", err);
      return res.status(500).json({
